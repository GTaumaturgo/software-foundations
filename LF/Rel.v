...
Theorem total_no_partial :
  ~(partial_function total_relation).
Proof. unfold not. unfold partial_function. intros H.
  Admitted.

(** **** Exercise: 2 stars, optional (empty_relation_partial)  *)
(** Show that the [empty_relation] assert (0 = 1) as Nonsense. {
    apply Hc with (x:=1).
    - apply reduce_swap. apply reduce_succ. apply reduce_same.
    - apply reduce_same.
  }
  inversion Nonsense.that we defined earlier is a
    partial function. *)
...
Proof.
  ...
  - intros H. apply IHn'. apply le_S_n. apply H.
Qed.
(** [] *)
...
Theorem le_not_symmetric :
  ~ (symmetric le).
Proof.
    unfold symmetric. intros H. assert (1 <= 0). {
...
Proof.
  ...
  - intros b H1 H2. destruct b as [|b'].
    + inversion H1.
    + assert (a = b'). {
      apply IHa.
...
Theorem le_step : forall n m p,
  ...
  n <= p.
Proof.
  unfold "<". intros n m p H1 H2. apply le_S_n. apply le_trans with m.
  - apply H1.
  - apply H2.
Qed.
(** [] *)
