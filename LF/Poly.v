...
Definition append_list_pair {X Y : Type} (lp1 : (list X) * (list Y)) (lp2 : (list X) * (list Y)) : (list X) * (list Y) :=
...
(**    ( append_list_pair  ((filter (test h) l ), (partition test t) )    , append_list_pair   ( filter (fun n => negb ( test h))) )   **)
...
Proof.
  intros X Y Z f. intros x y. reflexivity.
Qed.
...
Proof.
  intros X Y Z f. intros p . destruct p. reflexivity. 
Qed.
